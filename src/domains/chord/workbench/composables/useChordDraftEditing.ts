 ...
 * 抽屉通过 draftStore 参数注入自己的独立草稿实例（与工作台互不联动）。
 */
export const useChordDraftEditing = (draftStore: ChordEditorStore = useActiveChordEditorStore()) => {
 ...
 */
export const useChordDraftSaveState = (draftStore: ChordEditorStore = useActiveChordEditorStore()) => {
  const editorStore = draftStore;
