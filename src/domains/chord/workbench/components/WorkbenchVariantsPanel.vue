<template>
  ...
  >
    <template #default="{ effectiveExpanded }">
      <div v-if="hasVariants" class="pt-2">
        <div class="relative w-full">
          ...
          <div
            ...
            class="no-scrollbar flex w-full items-stretch gap-4 overflow-x-auto px-1 py-1 select-none"
            ref="scrollRef"
          >
            <div
              v-wave
              ...
              :class="[
                isActiveVariant(variant)
                  ? 'border-primary bg-tint-primary-90! ring-primary/40 ring-1 ring-inset'
                  : 'border-border-light bg-bg-body hover:border-border-base hover:bg-bg-panel-hover',
              ]"
              :key="variantKey(variant, index)"
              @click="handleSelectVariant(variant)"
              class="duration-fast group box-border flex shrink-0 cursor-pointer flex-col items-center rounded-md border-2 p-1.5 transition-colors"
            >
