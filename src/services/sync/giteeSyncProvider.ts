...
export function createGiteeSyncProvider(config: GiteeSyncConfig): SyncBranchesProvider {
  ...
  return {
    async pull() {
      ...
      if (!response.ok) throw new SyncError('REQUEST_FAILED', `Gitee 返回错误状态码：${response.status}`);
    ...
    async exists() {
      ...
      throw new SyncError('REQUEST_FAILED', `Gitee 返回错误状态码：${response.status}`);
    },
    async push(payload) {
      ...
      if (existing.ok) {
        ...
        throw new SyncError('REQUEST_FAILED', `Gitee 返回错误状态码：${existing.status}`);
      }
