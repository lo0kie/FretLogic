<template>
  <BasePopover
    ...
    match-trigger-width
    :block="width === 'full'"
    :panel-style="{ padding: '0px', overflow: 'hidden' }"
  >
    <template #trigger="{ isOpen: _isOpen, toggle }">
      ...
      >
        ...
        />
        <ChevronDown :size="14" :stroke-width="2.5" class="arrow-icon" :class="{ 'rotate-180': _isOpen }" />
      </div>
    </template>
...
const isOptionDisabled = (opt: T | SelectorOptionObject<T>): boolean => {
  if (opt && typeof opt === 'object' && 'disabled' in opt) return Boolean(opt.disabled);
  return false;
};
...
const canClear = computed(() => {
  if (defaultValue !== undefined) return modelValue.value !== defaultValue;
...
const handleSelect = (option: T | SelectorOptionObject<T>, closeFn: () => void) => {
...
const handleDropdownKeydown = (e: KeyboardEvent, closeFn: () => void) => {
...
/* 核心滚动区域，移除在 BasePopover 里已经存在的边框和高斯模糊，避免样式重复 */
...
@include fade-scale-transition(hint-fade);
</style>
