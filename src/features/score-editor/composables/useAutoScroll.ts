...
/** 延迟获取 scoreEditor 单例（模块加载期调用 useStore 可能早于 Pinia 安装） */
...
/** rAF 步进：按 scrollSpeed 换算每帧位移滚动容器，无可滚内容或触底时自动停止 */
...
/** 开始自动滚动（未注册容器时回退查找 .interactive-score-zone） */
...
/** 自动滚动装配：注册容器引用与用户滚动打断（wheel/touchstart），切歌/切页/卸载时自动停止 */
