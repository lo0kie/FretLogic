      ...
      <ul v-if="references.length > 0" class="references-list no-scrollbar">
        <li v-for="item in references" :key="item.song.id">
          ...
          >
            ...
            <span class="reference-key">{{ computeSongKey(item.song.playKey, item.song.capo) }}</span>
...
import BaseMarquee from '@/ui/components/BaseMarquee.vue';
import BaseModal from '@/ui/components/BaseModal.vue';
import BaseSegmentedControl from '@/ui/components/BaseSegmentedControl.vue';
import BaseSelector from '@/ui/components/BaseSelector.vue';
import EmptyState from '@/ui/components/EmptyState.vue';
import Fretboard from '@/ui/components/Fretboard.vue';
import type { useChordGroupModals } from '@/ui/composables/useChordGroupModals';
import { useGridNavigation } from '@/ui/composables/useGridNavigation';
import { KEY_OPTIONS, SORT_RULE_CONFIG, computeSongKey } from '@/utils/musicTheory';
