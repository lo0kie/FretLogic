 ...
 *
 * 去重：同一 key 连续失败只上报一次 —— 防抖写会按 400ms 节奏反复失败，
 * 不去重会刷屏。写入成功后由调用方 clearPersistFailure 解锁，使后续再次失败能重新提示。
 */
...
/** 上报一次写入失败：始终落日志，同键仅在首次失败时通知订阅方；配额超限额外触发熔断。 */
