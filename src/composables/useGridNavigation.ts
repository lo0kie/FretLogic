...
export function useGridNavigation(
  ...
  const resolveCols = (): number | undefined => {
    if (cols === undefined || cols === null) return undefined;
    if (typeof cols === 'function') return (cols as () => number)();
    if (typeof cols === 'object') return (cols as Ref<number>).value;
    return cols;
  };
  ...
  const handleKeydown = (e: KeyboardEvent) => {
    ...
    switch (e.key) {
      ...
      case 'ArrowUp': {
        ...
        if (c && c > 1) {
          const targetIdx = currentIndex - c;
