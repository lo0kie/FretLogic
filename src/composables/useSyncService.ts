...
import { validateGithubSettings, validateWebdavSettings } from '@/utils/validateSettings';
import { ref } from 'vue';
...
export function useSyncService() {
  ...
  const resolveProvider = (errorPrefix: string): SyncProvider | null => {
    if (settingsStore.syncTarget === 'webdav') {
      const validation = validateWebdavSettings({
      ...
      const { webdavServerUrl, webdavUsername, webdavPassword, webdavProxyUrl } = validation.data;

      return createWebdavSyncProvider({
        kind: 'webdav',
        serverUrl: webdavServerUrl,
        username: webdavUsername,
        password: webdavPassword,
        proxyUrl: webdavProxyUrl || undefined,
      });
    }
    ...
    return createGithubSyncProvider({
      kind: 'github',
      token: githubToken,
      owner: githubOwner,
      repo: githubRepo,
  ...
  const fetchGithubBranches = async (): Promise<boolean> => {
    ...
    if (!provider || !('listBranches' in provider)) return false;
