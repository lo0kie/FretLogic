 ...
 * - 四个导出动作（长图/PDF/Zip/打印）的完整实现移入 scoreExportHandlers.ts，经动态 import
...
export const useScoreExportActions = () => {
  ...
  const handleScoreExport = (op: 'copy' | 'download') => loadHandlers().then(m => m.handleScoreExport(op));
  ...
  const downloadExportMenuItems: MenuItem[] = [
    {
      label: '下载为长图',
      icon: 'image-down',
      action: () => void loadHandlers().then(m => m.handleScoreExport('download')),
    },
    {
      label: '下载为 PDF',
      icon: 'file-text',
      action: () => void loadHandlers().then(m => m.handleScoreExportPdf()),
    },
    {
      label: '下载为 ZIP',
      icon: 'file-archive',
      action: () => void loadHandlers().then(m => m.handleScoreExportZip()),
    },
    {
      label: '打印',
      icon: 'printer',
      ...
      divided: true,
      action: () => void loadHandlers().then(m => m.handleScorePrint()),
    },
  ];
