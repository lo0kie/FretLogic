 ...
 * 编辑点越靠前、并发改动越容易撞在同一段。拆成「一次工作区（一次提交）一个片段」后：
 * 新增条目 = 新增一个小文件，既不触碰既有文件、也不产生折行重排，
 * 于是 diff 恒为「N 行新增 / 0 行删除」，也不会有合并冲突。
 *
 * 分区规则：**以工作区为界**。已提交的片段即冻结（只读 —— 改它就把「N 行新增 / 0 行删除」破坏了）；
 * 未提交的改动可塑 —— 工作区里的全部未提交改动同属一区，片段边界对齐**工作区**、不对齐「改了几轮」、
 * 也不按主题拆：一次工作区的改动一律并进「当前工作区片段」（上次提交之后创建、尚未提交的那个）、就地
 * 改写，而不是每改一轮或每换一个主题新开一个片段。生成器本身不区分这两区
 * （它只是个拼接器），这条约束由作者与 `rules/04-changelog-fragment-zones.md` 的「一、变更日志片段的分区规则」保证。
 *
