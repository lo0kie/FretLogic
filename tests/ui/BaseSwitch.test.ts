...
describe('BaseSwitch.vue', () => {
  it('renders correctly with default props', () => {
    ...
    const button = wrapper.find('button.base-switch');
    expect(button.exists()).toBe(true);
    expect(button.attributes('role')).toBe('switch');
    expect(button.attributes('aria-checked')).toBe('false');
    expect(button.classes()).not.toContain('is-checked');
    expect(wrapper.text()).toContain('开关测试');
  });
  ...
  it('emits update:modelValue and change on pointer click/drag', async () => {
    ...
    const btn = wrapper.find('button.base-switch');
